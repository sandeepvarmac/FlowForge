"""
Silver Dataset Job Task

Layer-centric Prefect task that transforms multiple Bronze inputs into a Silver output.
Uses SQL transformation with DuckDB for cross-source joins.
"""
...
from utils.parquet_utils import add_surrogate_key, write_parquet
...
def _build_silver_dataset_key(
    output_table_name: str,
    run_id: str,
) -> tuple[str, str]:
    """
    Build S3 keys for Silver dataset job output.
    ...
    # Use output_table_name for folder structure
    current_filename = f"{output_table_name}_{run_id}.parquet"
    current_key = f"silver/{output_table_name}/{date_folder}/{current_filename}"

    archive_filename = f"{timestamp}_v001.parquet"
    archive_key = f"silver/{output_table_name}/{date_folder}/archive/{archive_filename}"
...
def _extract_s3_key_from_path(file_path: str) -> str:
    ...
    Args:
        file_path: Full path like 's3://flowforge-data/bronze/...'

    Returns:
        Just the key portion: 'bronze/...'
    """
...
@task(name="silver_dataset_job", retries=1, retry_delay_seconds=30)
def silver_dataset_job(
    ...
    *,
    ...
    """
    Execute a Silver layer Dataset Job that joins/transforms multiple Bronze inputs.

    This is a layer-centric task that:
    1. Downloads multiple Bronze datasets from S3
    2. Loads them into DuckDB as named tables
    3. Executes the SQL transform
    4. Writes output to Silver layer
    5. Catalogs with multiple parent_tables for lineage

    Args:
        input_datasets: List of Bronze table names to use as inputs
        transform_sql: SQL query to transform/join the inputs
        output_table_name: Name for the output Silver table
        source_id: Optional source ID (for tracking)
        execution_id: Optional execution ID
        environment: Environment (dev, qa, uat, prod)
        silver_config: Optional Silver layer configuration

    Returns:
        Dict with output metadata including silver_key, records, columns
    """
    ...
    logger.info("=" * 60)
    logger.info("🔷 SILVER DATASET JOB")
    logger.info("=" * 60)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp_dir:
            ...
            try:
                ...
                for table_name in input_datasets:
                    ...
                    # Register as a DuckDB table (using the table name from catalog)
            ...
            logger.info("\n📋 Step 5: Adding surrogate key...")
            result_df = add_surrogate_key(result_df, key_column="_sk_id", start=1)

            # Step 6: Write to Silver layer
            logger.info("\n📋 Step 6: Writing to Silver layer...")
            current_key, archive_key = _build_silver_dataset_key(output_table_name, run_id)
            ...
            asset_id = catalog_dataset_job_output(
                ...
                description=f"Silver Dataset Job: {output_table_name} (from {', '.join(input_datasets)})",
        ...
        # Success!
        logger.info("\n" + "=" * 60)
        logger.info("✅ SILVER DATASET JOB COMPLETE")
        logger.info("=" * 60)
