...
import { ViewJobDetailsModal } from './view-job-details-modal'
import { useJobActions } from '@/hooks'
import { Job } from '@/types/workflow'
import { FileText, Database, Cloud, Play, Loader2, CheckCircle, ArrowRight, Layers } from 'lucide-react'
...
export function JobCard({ job, workflowId, onRunJob, onEditJob, onCloneJob, isRunning }: JobCardProps) {
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-start gap-3 flex-1">
            <div className="w-8 h-8 bg-primary-50 rounded-lg flex items-center justify-center mt-1">
              ...
              {job.type === 'nosql' && <Database className="w-4 h-4 text-primary" />}
              {job.type === 'api' && <Cloud className="w-4 h-4 text-primary" />}
              {job.type === 'gold-analytics' && <Layers className="w-4 h-4 text-primary" />}
            </div>
