...
export const CSVFileUpload = React.forwardRef<{ reset: () => void }, CSVFileUploadProps>(
  ...
  // Primary key detection is now handled by AI Schema Intelligence (/api/ai/analyze-schema)
  // The AI can semantically understand the difference between primary keys and foreign keys
  // Returns empty array - actual PK detection happens via AI after file upload
  const detectPKCandidates = (_schema: Array<{ name: string; type: string }>): string[] => {
    // Deprecated: Algorithm-based detection replaced with AI-powered Schema Intelligence
    // Returns empty - create-job-modal calls fetchSchemaIntelligence() for accurate results
    return []
  }
