...
/**
 * PATCH /api/workflows/:workflowId/triggers/:triggerId/enable
 * Enable a trigger
 */
export async function PATCH(
  ...
  try {
    ...
    // For scheduled triggers, sync with Prefect (resume deployment)
    if (trigger.trigger_type === 'scheduled' && trigger.cron_expression) {
      try {
        ...
        await execFileAsync(
          pythonExecutable,
          [scriptPath, 'resume', triggerId],
          { timeout: 15000 }
        )

        console.log(`Resumed Prefect deployment for trigger ${triggerId}`)
      } catch (error) {
