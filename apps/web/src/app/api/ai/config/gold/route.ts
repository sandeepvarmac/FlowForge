...
export async function POST(request: NextRequest) {
  try {
    ...
    const { dbType, connection, tableName, connectionId, businessContext } = body

    console.log('[Gold AI API] Request received:', { dbType, tableName, connectionId: connectionId ? 'provided' : 'missing', businessContext: businessContext ? 'provided' : 'not provided' })

    // Validate required fields
    if (!dbType || !tableName) {
      console.error('[Gold AI API] Validation failed - missing required fields')
